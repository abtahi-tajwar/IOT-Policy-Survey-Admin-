...
function Candidates() {
  ...
  const [selectedScenarioGroup, setSelectedScenarioGroup] =
    React.useState("all");
  ...
  React.useEffect(() => {
    ...
    scenarioGroup.get().then((scenarioGroupRes) => {
      ...
      if (selectedScenarioGroup === "all") {
        ...
        candidate
          .get(10, [where("assignedGroup", "==", selectedScenarioGroup)])
          .then((res) => {
            setCandidates(res.response);
            updateTableData(res.response, scenarioGroupRes.response);
            setIsLastPage(res.isLastPage);
  ...
  const updateTableData = (candidates, scenarioGroups) => {
    setTableData(
      candidates.map((candidate) => ({
        ...
        scenarioGroup:
          scenarioGroups.find((sg) => sg.id === candidate.data.assignedGroup)
            ?.data.name ?? "No Group Was Assigned",
      }))
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (rowData) => (
        <IconButton
          LinkComponent={Link}
          to={`/candidate/responses/${rowData.id}`}
          color="primary"
          variant="contained"
        >
          <RemoveRedEyeIcon />
    ...
    <div>
      <Loader isLoading={loading}>
        <Grid container justifyContent={'space-between'} alignItems={'center'}>
          ...
          <Grid item>
            <Button variant="contained" color="danger">Delete All Candidate Responses</Button>
          </Grid>
        </Grid>
